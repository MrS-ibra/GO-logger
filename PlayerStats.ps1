...
try {
    ...
    $count = ($html -split "Total Lifetime Players:")[1] -split "<"[0]
    ...
    $joinedToday = ($peakTodayLines.Count -ge 2) ?
        [int]::Parse(($peakTodayLines[-1] -split ",")[2]) - [int]::Parse(($peakTodayLines[0] -split ",")[2]) : 0

    $previousCount = ($peakTodayLines.Count -ge 2) ?
        [int]::Parse(($peakTodayLines[-2] -split ",")[2]) : [int]::Parse($count)
