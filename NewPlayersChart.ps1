...
.SYNOPSIS
  ...
  (up to 7 days back), split into a minimum of 20 slots, with data labels,
  a chart title, and your Imgur logo as a watermark.
.PARAMETER Slots
...
# 2. Time window (cap at 7 days)
$latest        = $entries[-1].DateTime
$earliest      = $entries[0].DateTime
$minStart      = $latest.AddDays(-7)
$startTime     = if ($earliest -lt $minStart) { $minStart } else { $earliest }
...
for ($i = 0; $i -lt $Slots; $i++) {
  ...
  # Label: date+time if >1 day, else time only
...
# 5. QuickChart JSON (title + datalabels + watermark)
$chartConfig = @{
  type    = 'bar'
  data    = @{
    labels   = $labels
    datasets = @(@{ data = $data; backgroundColor = 'rgba(54,162,235,0.7)' })
  }
  options = @{
    title = @{
      display = $true
...
# 6. Render chart at 1200×600 with both plugins
$enc     = [uri]::EscapeDataString($chartConfig)
$plugs   = 'chartjs-plugin-datalabels,chartjs-plugin-watermark'
$uri     = "https://quickchart.io/chart?c=$enc&width=1200&height=600&format=png&plugins=$plugs"
...
Invoke-RestMethod -Uri $WebhookUrl `
  ...
  -Form @{ payload_json = $payloadJson; file1 = Get-Item $ChartFile }

Write-Host "✅ New-players chart posted with $Slots slots (cumulative = $cumulative)."
