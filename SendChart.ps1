...
.SYNOPSIS
  ...
  (up to 7 days), splits that range into 18 equal slots, shows counts on each bar,
  applies your Imgur logo as a watermark, and posts to Discord.
.PARAMETER Slots
...
# 2. Determine dynamic window (cap at 7 days)
...
# 6. Fetch the chart PNG (enable both plugins)
$encoded = [System.Net.WebUtility]::UrlEncode($chartConfig)
$uri     = "https://quickchart.io/chart?c=$encoded&width=900&height=400&format=png&plugins=chartjs-plugin-datalabels,chartjs-plugin-watermark"
Invoke-WebRequest -Uri $uri -OutFile $outputImage

# 7. Post to Discord (embed only, keep Spy Drone)
...
Write-Host "Chart sent: $Slots slots over $([math]::Round($totalDuration.TotalDays,2)) days (cumulative=$cumulative)."
